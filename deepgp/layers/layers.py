...
class ClassificationObservedLayer(Model):

    def __init__(self, dim_down, dim_up, Y, X=None, X_variance=None, obs_data='cont', epsilon=1e-3, a=3., b = 0., init='rand', mpi_comm=None, mpi_root=0, back_constraint=True, encoder=None, mmlayer=False, auto_update=True, name='obslayer'):
        super(ClassificationObservedLayer, self).__init__(name)
        self.auto_update = auto_update
        self.qU_ratio = 1.
        if obs_data=='binary':
            self.bin_data = True
            assert dim_down==dim_up
            self.nClasses = 2
        elif obs_data=='classification':
            self.bin_data = False
            assert dim_down==1
            self.nClasses = dim_up
        else:
            raise ValueError('The type of observed data has to be "binary" or "classification"!')
        ...
        if self.bin_data:
            from ..inference import BinaryClassInference
            self.inference_method = BinaryClassInference()
        else:
            from ..inference import MultiClassInference, MultiClass_SVI
            self.inference_method = MultiClass_SVI() #MultiClassInference()
        
        if self.bin_data:
            self.a = Param('alpha',a, Logexp())
            self.b = Param('beta', b, __fixed__)
            self.link_parameters(self.a, self.b)
        else:
            self.a = Param('alpha',a, __fixed__)
            self.link_parameter(self.a)
            # self.epsilon = Param('epsilon',epsilon, __fixed__)
            # self.link_parameter(self.epsilon)
    ...
    def parameters_changed(self):
        if not self.mmlayer and self.auto_update: self.update_layer()
        
    def update_layer(self):
        if self.bin_data:
            self._log_marginal_likelihood, self.grad_dict = self.inference_method.inference(self.X, self.Y, self.a, self.b)
        else:
            self._log_marginal_likelihood, self.grad_dict = self.inference_method.inference(self.X, self.Y, self.a)#self.epsilon)
        if self.mpi_comm is not None and not self.mmlayer:
            L = self.mpi_comm.reduce(self._log_marginal_likelihood, root=self.mpi_root)
            if self.mpi_comm.rank==self.mpi_root: self._log_marginal_likelihood = L
        self.X.mean.gradient[:] = self.grad_dict['dL_dX_mean']
        self.X.variance.gradient[:] = self.grad_dict['dL_dX_var']
        if self.uncertain_inputs and not self.mmlayer:
            delta = -self.variationalterm.comp_value(self.X)
            if self.mpi_comm is not None:
                delta = reduceArrays([np.float64(delta)],self.mpi_comm, self.mpi_root)[0]
                if self.mpi_comm.rank != self.mpi_root: delta = 0.
    ...
    def predict(self, X):
        if self.bin_data:
            return self.inference_method.predict(X, self.a, self.b)
        else:
            return self.inference_method.predict(X, self.a)#self.epsilon)
    ...
    @property
    def is_fixed(self):
        # The hack to force the "parameters_changed" being executed.
        return False
